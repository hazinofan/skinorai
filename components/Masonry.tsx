...
const Masonry: React.FC<MasonryProps> = ({
  ...
  useEffect(() => {
    let cancelled = false;
    const timeoutId = window.setTimeout(() => setImagesReady(false), 0);
  ...
  useLayoutEffect(() => {
    ...
    if (revealOnScroll) gsap.registerPlugin(ScrollTrigger);
    ...
    if (revealOnScroll) window.requestAnimationFrame(() => ScrollTrigger.refresh());
        ...
        >
          <div
            className={`relative w-full h-full bg-cover bg-center rounded-[10px] uppercase text-[10px] leading-[10px] ${
              itemShadow ? 'shadow-[0px_10px_50px_-10px_rgba(0,0,0,0.2)]' : ''
            }`}
